...
export async function getIndexPage(req, res, next) {
  try {
    const result = await getLections("teorija-sistema");
    ...
    return res.status(200).render("subjects/teorija-sistema/index", {
...
export async function getLectionDataPage(req, res, next) {
  try {
    ...
    return res
      .status(200)
      .render(
        `subjects/teorija-sistema/lections/${viewLection.template}`,
        {
...
export async function getMaterialsPage(req, res, next) {
  try {
    const result = await findAllAvailablePdfs("teorija-sistema");
    ...
    res.render("subjects/teorija-sistema/materials", result);
  } catch (error) {
    next(error);
  }
}
